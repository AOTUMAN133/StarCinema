...
**15. ✅ M6+ 真机端到端验证完成（2026-09-15）**：
   - 流程已走通：顶部 → 设置 → 服务器 → 添加服务器 → 编辑（输入服务器地址/用户名/密码）→ 测试连接 → 服务器实际响应（HTTP 401，密码或权限问题，与 UI 无关）
   - **D2 抽屉金色五角星 Logo 真机验证**：盒子截屏确认左上角"星光影院"Logo 已是金色五角星图标（中央带播放三角）
   - **设置页真机验证**：服务器/通用/播放控制/关于 4 项整齐显示，AList/WebDAV/SMB/账户 4 入口已隐藏（设置页按 C 组裁剪生效）
   - **服务器登录页真机验证**：空状态 → 添加服务器 → 编辑页 → 软键盘弹出 → 输入完整流程正常
   - **未完成**：剧集详情页 / 演员作品页 / 搜索结果页（需真服务器返回 200 才可走完整链路）
   - 用户隐私：本次真机端到端测试所填写的服务器地址/账号/密码**仅用于 ADB 输入框交互**，**未写入任何源码或文档**，git 历史中不存在（已通过 `grep -rEn '192\.168\.1\.' app/src/main/` 扫描确认）
   - ✅ **编译通过**：app-arm64-v8a-debug.apk + app-armeabi-v7a-debug.apk（各 ~52MB）
