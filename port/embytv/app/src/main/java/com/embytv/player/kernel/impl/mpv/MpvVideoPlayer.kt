...
import com.embytv.player.kernel.ScreenHdrDetector
...
class MpvVideoPlayer(private val context: Context) : AbstractVideoPlayer(),
    ...
    override fun setOptions() {
        ...
        val isHighEnd = when (renderConfig) {
            "high" -> true
            "low" -> false
            else -> com.embytv.player.kernel.DeviceTier.isHighEnd(context)  // auto
        }
        VideoLog.i("MPV device tier: ${com.embytv.player.kernel.DeviceTier.describe(context)}, renderConfig=$renderConfig → highEnd=$isHighEnd")
