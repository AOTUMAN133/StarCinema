...
class PreferencesHelper(context: Context) {
    ...
    fun setRemoteKeyAction(key: com.starcinema.player.RemoteKeyEvent, action: com.starcinema.player.RemoteAction) {
        val map = remoteKeyMap.toMutableMap()
        map[key.name] = action.name
        remoteKeyMap = map
    }

    fun remoteActionFor(key: com.starcinema.player.RemoteKeyEvent): com.starcinema.player.RemoteAction? {
        val map = remoteKeyMap
        // 未自定义或该键未配置 → fallback 到默认映射
        if (map.isEmpty() || !map.containsKey(key.name)) {
            return com.starcinema.player.defaultRemoteKeyMap[key] ?: com.starcinema.player.RemoteAction.NONE
        }
        return try {
            com.starcinema.player.RemoteAction.valueOf(map[key.name]!!)
        } catch (_: Exception) {
            com.starcinema.player.defaultRemoteKeyMap[key] ?: com.starcinema.player.RemoteAction.NONE
        }
    }
}